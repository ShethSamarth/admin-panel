...
export const FabricForm = ({ initialData }: FabricFormProps) => {
  ...
  const description = initialData ? "Edit a fabric" : "Add a new fabric"
  ...
  const onSubmit = async (data: FabricFormValues) => {
    try {
      if (initialData) {
        await axios.patch(`/api/fabrics/${params.fabricId}`, data)
      } else {
  ...
  const onDelete = async () => {
    try {
      setLoading(true)
      await axios.delete(`/api/fabrics/${params.fabricId}`)
      router.push("/fabrics")
      router.refresh()
      toast.success("Fabric deleted.")
    } catch (error) {
      toast.error("Make sure you removed all products using this fabric first.")
    } finally {
      setLoading(false)
      setOpen(false)
    }
